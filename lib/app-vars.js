...
const AppVars = {
    ...
    async init() {
        ...
        try {
            // cek awal fetching
            if (this.isFetching) {
                console.log("Fetch sedang berjalan, ditahan dulu...");
                return;
            }

            console.log("Memulai fetching...");
            ...
            if (supportedData && templates.supported) {
                this.renderList(supportedData, 'supported', templates.supported);
            }
            ...
            if (teamData) {
                ...
                if (templates.team_index) {
                    this.renderList(activeTeam, 'team_index', templates.team_index);
                }
                if (templates.team_portofolio) {
                    this.renderList(activeTeam, 'team_portofolio', templates.team_portofolio);
                }
                if (templates.panitia_zisam) {
                    this.renderList(activeTeam, 'panitia_team', templates.panitia_zisam)
                }
            }
